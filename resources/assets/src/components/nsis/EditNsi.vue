<template>
    <div>
      <b-modal @ok="handleOk" size="xl" id="modal-editnsi" title="Редактирование источника НСИ" ok-title="Сохранить" cancel-title="Закрыть">
          <div>
            <b-form-group label-size="lg" label-cols="2" description="Выберите тип источника" label="Тип источника *" label-for="nsi_type">
            <b-form-select id="nsi_type" v-model="new_nsi.type_id" :options="types" value-field="id" text-field="name"></b-form-select>
            </b-form-group>
            <div v-if="new_nsi.type_id==1">
...
export default {
  ...
  methods: {
       handleOk(bvModalEvt) {
        ...
        this.errors = []
        if (this.new_nsi.type_id.length == 0) { this.errors.push("Не выбран тип источника")}
  ...
  mounted () {
      axios
      .get('/nsis/get_nsi/'+this.nsi_id)
      .then((response) => (this.new_nsi = response.data))
  }
}
</script>